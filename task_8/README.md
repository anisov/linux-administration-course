...
**Цель** :
1) Отрабатываем навыки работы с созданием томов export/import и установкой параметров. 
2) Определить алгоритм с наилучшим сжатием. 
3) Определить настройки pool’a.
4) Найти сообщение от преподавателей.
...
1) vagrant up .
2) Запустить скрипт `sudo /vagrant/./script.sh`.

### Описание.
1) `start.sh` Устанавливает zfs и перезагружает систему при vagrant up.
2) `script.sh` Описывает и запускает последовательность действий для выполнения необходимых заданий.
