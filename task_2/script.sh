...
mkdir -p /md_disks/part_{1,2,3,4,5}
for i in $(seq 1 5); do mkfs.ext4 /dev/md10p$i; done
for i in $(seq 1 5); do mount /dev/md10p$i /md_disks/part_$i; done
for i in $(seq 1 5); do echo "/dev/md10p$i /md_disks/part_$i ext4 defaults 0 0" >> /etc/fstab; done
